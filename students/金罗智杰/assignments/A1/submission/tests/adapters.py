...
def run_linear(
    ...
    layer = Linear(d_in=d_in, d_out=d_out, device=weights.device, dtype=weights.dtype)
    with torch.no_grad():
...
def run_swiglu(
    ...
    layer = SwiGLU(d_model=d_model, d_ff=d_ff, device=in_features.device, dtype=in_features.dtype)
    with torch.no_grad():
